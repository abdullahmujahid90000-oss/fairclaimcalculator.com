...
| ⚠️ | Mobile/accessibility checks. Upgraded 2026-08-08 from a simple structural regex pass to a real automated WCAG rule-engine scan (axe-core, run headlessly via `astro/scripts/axe-check.mjs` against every built page). Result: **0 violations across all 81 pages** (the 54 Astro-routed pages plus, for the first time, all 27 legacy redirect/retired-content stub pages under `public/` — a coverage gap discovered and fixed this pass, see BUILD-LOG.md). Found and fixed 2 real issues in the process: an unlabeled table header cell, and ~30 stub pages missing a landmark region (`<main>`) around their body content — the latter fixed at the generator-template level (`generate-legacy-stubs.mjs`) so it survives future rebuilds. Color-contrast is not checked by this script (jsdom doesn't do real layout/rendering) but was separately verified by hand against the site's fixed color palette. **Still outstanding:** the actual manual keyboard-only and screen-reader walkthrough at 320/390/768/1024/1440px, which needs a live URL — planned for immediately after cutover. |
